    ...
    return <>
                    ...
                    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="max-w-2xl">
                      ...
                      <span className="bg-white/80 backdrop-blur-sm border border-white/50 text-[#102a43] px-4 py-1.5 rounded-full text-sm font-bold flex items-center gap-2 mb-6 shadow-sm">
                        ...
                        <motion.div variants={itemVariants} className="flex flex-wrap gap-4">
                            ...
                            <Link to="/products?compare_price=true"> {/* Changed navigation to filter for sales */}
        ...
        <section className="py-24 bg-white relative">
            <div className="container mx-auto px-4">
                ...
                {/* Updated grid to 3 columns on mobile */}
                <div className="grid grid-cols-3 md:grid-cols-3 gap-4 md:gap-8">
                    {displayCategories.map((category, idx) => <motion.div key={category.id} initial={{
                        ...
                        <Link to={`/products?category=${category.slug}`} className="block group">
                            <div className="relative aspect-[4/5] md:aspect-[3/4] rounded-2xl overflow-hidden mb-2 md:mb-4 shadow-md group-hover:shadow-xl transition-all duration-500">
                                ...
                                <div className="absolute bottom-2 left-2 md:bottom-6 md:left-6 text-white">
                                    <p className="font-bold text-sm md:text-3xl mb-0 md:mb-2">{category.name}</p>
                                    <span className="text-sm font-medium border-b border-white pb-1 opacity-0 group-hover:opacity-100 transition-opacity duration-300 hidden md:inline-block">Shop Now</span>
                                </div>
                            </div>
                        </Link>
                    </motion.div>)}
        ...
        <section className="py-24 bg-slate-900 text-white overflow-hidden relative">
            ...
            <div className="container mx-auto px-4 relative z-10">
                ...
                {featuredProducts.length > 0 ? (
                    // Updated grid to 3 columns on mobile
                    <div className="grid grid-cols-3 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8">
                        {featuredProducts.map((product, idx) => <motion.div key={product.id} initial={{
                            ...
                            <Link to={`/products/${product.slug}`}>
                                <div className="relative aspect-[4/5] bg-slate-800 rounded-xl md:rounded-3xl overflow-hidden mb-2 md:mb-4 shadow-2xl">
                                    ...
                                    <div className="absolute top-2 right-2 md:top-4 md:right-4 bg-black/40 backdrop-blur-md text-white text-[10px] md:text-xs font-bold px-2 py-0.5 md:px-3 md:py-1 rounded-full border border-white/20">
                                ...
                                <div className="space-y-1">
                                    <h3 className="font-bold text-xs md:text-lg truncate text-slate-100">{product.name}</h3>
                                    <div className="flex flex-col md:flex-row gap-1 md:gap-3 items-start md:items-center">
                                        <span className="text-blue-200 font-bold text-sm md:text-xl">{formatCurrency(product.price)}</span>
                                        {product.compare_price && <span className="text-slate-600 line-through text-xs md:text-sm">{formatCurrency(product.compare_price)}</span>}
                                    </div>
                                </div>
                            </Link>
                        </motion.div>)}
                    ...
                    {loading ? (
                        ...
                        newArrivals.map((product) => (
                            <Link key={product.id} to={`/products/${product.slug}`} className="group block">
                                <div className="mb-2 md:mb-4 relative rounded-xl md:rounded-2xl overflow-hidden aspect-square bg-slate-100">
                                    ...
                                    <div className="absolute top-2 left-2 md:top-3 md:left-3">
                                        <span className="bg-white/90 backdrop-blur-sm text-[10px] md:text-xs font-bold px-1.5 py-0.5 md:px-2 md:py-1 rounded-md text-slate-900 shadow-sm">NEW</span>
                                    </div>
                                </div>
                                <div>
                                    <h3 className="font-medium text-xs md:text-base text-slate-900 mb-0.5 md:mb-1 group-hover:text-blue-600 transition-colors line-clamp-1">{product.name}</h3>
                                    <p className="text-slate-500 text-xs md:text-sm font-semibold">{formatCurrency(product.price)}</p>
                                </div>
                            </Link>
                        ))
                    )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {testimonials.map((t, i) => (
                        <motion.div
                            key={t.id ?? i}
                            whileHover={{ y: -8 }}
                            className="bg-white p-6 md:p-8 rounded-[2rem] shadow-lg shadow-indigo-100 border border-slate-100"
                        >
            ...
            <div className="container mx-auto px-4">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
                    {[{
                        ...
                        <div className="w-16 h-16 md:w-20 md:h-20 bg-slate-50 rounded-[1.5rem] flex items-center justify-center mb-6 group-hover:bg-[#102a43] group-hover:shadow-lg group-hover:shadow-blue-900/20 transition-all duration-300">
                            <item.icon className="w-6 h-6 md:w-8 md:h-8 text-slate-400 group-hover:text-white transition-colors duration-300" />
                        </div>
                        <h3 className="font-bold text-lg md:text-xl mb-2 text-slate-900">{item.title}</h3>
